...
async def test_global_bearer_allowlist_not_bypassed_by_cached_implementation_id():
    ...
    assert await first.headers() == {"Authorization": "Bearer glob"}
...
async def test_host_matched_spec_not_sent_over_plain_http(tmp_path, monkeypatch):
    ...
    assert await resolver.resolve({"serviceInfoUrl": "https://drs.test/x"}).headers() \
        == {"Authorization": "Bearer s3cret"}
    assert await resolver.resolve({"serviceInfoUrl": "http://drs.test/x"}).headers() == {}
    await http.aclose()
...
async def test_loopback_http_still_allowed_for_local_development(tmp_path, monkeypatch):
    ...
    assert await p.headers() == {"Authorization": "Bearer s3cret"}
    await http.aclose()
...
async def test_implementation_id_spec_honours_hosts_pin(tmp_path, monkeypatch):
    monkeypatch.setenv("TOK", "s3cret")
    resolver, http = _resolver(auth_config=_cfg(tmp_path, {
        "org.test.drs": {"kind": "bearer", "token_env": "TOK", "hosts": ["drs.test"]}}))
    ...
    assert await good.headers() == {"Authorization": "Bearer s3cret"}
    assert await bad.headers() == {}
    await http.aclose()
