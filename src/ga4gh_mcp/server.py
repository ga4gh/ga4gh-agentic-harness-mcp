...
# Registry and type-aware reads contact remote registries/services but change nothing.
...
_AUTH_WRITE = ToolAnnotations(readOnlyHint=False, destructiveHint=False, idempotentHint=False,
                              openWorldHint=True)
...
def build_server(settings: Settings | None = None,
    ...
    async def call_service_endpoint(service_id: str, path: str, method: str = "GET",
        ...
        Returns the structured envelope; on 401 it includes an auth hint. Only GET/HEAD are sent
        unless the operator set GA4GH_MCP_ALLOW_WRITE_METHODS=true."""
